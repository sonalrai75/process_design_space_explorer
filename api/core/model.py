...
class DesignVariable(BaseModel):
    ...
    def numeric_project(self, value: float) -> float:
        ...
        if self.kind == 'quantized':
            if not self.step or self.step <= 0: raise ValueError('step must be positive')
...
class ProcessModel(BaseModel):
    ...
    def activity_map(self): return {a.id:a for a in self.activities}
    def resource_map(self): return {r.id:r for r in self.resources}
